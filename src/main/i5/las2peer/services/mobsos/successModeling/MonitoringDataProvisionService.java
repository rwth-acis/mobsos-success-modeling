...
public class MonitoringDataProvisionService extends RESTService {
	...
	public MonitoringDataProvisionService() {
		...
		if (this.databaseType == SQLDatabaseType.MySQL) {
			...
			this.GROUP_QUERY = "SELECT GROUP_AGENT_ID,GROUP_NAME " + "FROM GROUP_INFORMATION " + "WHERE PUBLIC=1";
			...
			this.SERVICE_QUERY = "SELECT SERVICE.AGENT_ID,SERVICE_CLASS_NAME,SERVICE_PATH FROM " + DB2Schema
			...
			this.GROUP_QUERY = "SELECT GROUP_AGENT_ID,GROUP_NAME " + "FROM " + DB2Schema + ".GROUP_INFORMATION "
					+ "WHERE PUBLIC=1";
			this.GROUP_QUERY_WITH_ID_PARAM = this.GROUP_QUERY + " AND GROUP_AGENT_ID=?";
			...
			this.GROUP_INFORMATION_UPDATE = "UPDATE " + DB2Schema
			...
			for (String successModelFile : successModelFiles) {
				...
				if (measureCatalog != null) {
					...
					if (group != null) {
						...
						modelsV2.get(group).put(successModel.getServiceName(), successModel);
					}
				} else {
					System.out.println("No measure catalog found for model " + successModelFile);
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
	 ...
	 */
	protected MeasureCatalog updateMeasures(String measureFile) throws MalformedXMLException {

		String measureXML = getMeasureFile(measureFile);
		return readMeasureCatalog(measureXML);
	}

	private MeasureCatalog readMeasureCatalog(String measureXML) throws MalformedXMLException {
	...
	private SuccessModel readSuccessModelFile(String successModelXml, String measureFilePath)
		...
		for (Element dimensionElement : elements) {
			...
			case "Community Impact":
				...
				break;
			default:
				throw new MalformedXMLException("Dimension " + dimensionName + " is unknown!");
			}
				...
				if (dChildren.item(factorNumber).getNodeType() == Node.ELEMENT_NODE) {
					...
					for (int measureNumber = 0; measureNumber < fChildren.getLength(); measureNumber++) {
						if (fChildren.item(measureNumber).getNodeType() == Node.ELEMENT_NODE) {
							...
							if (!measureCatalogs.get(measureFilePath).getMeasures().containsKey(measureName)) {
								measureCatalogs.put(measureFilePath, updateMeasures(measureFilePath));
							}
							if (!measureCatalogs.get(measureFilePath).getMeasures().containsKey(measureName)) {
								throw new MalformedXMLException("Measure name " + measureName + " is unknown!");
							}
							factorMeasures.add(measureCatalogs.get(measureFilePath).getMeasures().get(measureName));
						}
					}
		...
		return new ServiceSuccessModel(modelName, serviceName, factors, successModelXml);
	}
	...
	*/
	...
	void writeMeasureCatalog(String xml, String group) throws MalformedXMLException, FileBackendException {
		readMeasureCatalog(xml);
		measureFileBackend.writeFile(Paths.get(group, "measure-catalog.xml").toString(), xml, group);
	...
	void writeSuccessModel(String xml, String group, String expectedServiceName)
		...
		if (!successModel.getServiceName().equals(expectedServiceName)) {
			throw new MalformedXMLException(
					"Service name is " + successModel.getServiceName() + " and not " + expectedServiceName);
		}
